...
def load_electrode_data(file_path):
    electrode_data = pd.read_csv(file_path, delim_whitespace=True, header=None, names=['x', 'y', 'z', 'name', 'weight', 'WMvsGM'])
    electrode_positions = electrode_data[['x', 'y', 'z']].values.astype(float)
    electrode_weights = electrode_data['weight'].values
    return electrode_data, electrode_positions, electrode_weights
...
for hemi in hemispheres:
    for analysis_type, title in files_and_titles:
        ...
        electrode_data, electrode_positions, electrode_weights = load_electrode_data(electrode_file)

        # Print coordinates before reflection
        print(f'Original {hemi} coordinates for {analysis_type}:')
        print(electrode_positions[:5])

        # Reflect coordinates for the right hemisphere
        if hemi == 'rh':
            electrode_positions[:, 0] = -electrode_positions[:, 0]

        # Print coordinates after reflection
        print(f'Reflected {hemi} coordinates for {analysis_type}:')
        print(electrode_positions[:5])
        ...
        save_path = os.path.join(base_path, 'figures/surfer-black-green', f'{analysis_type}_{num_elecs}_{hemi}.png')
