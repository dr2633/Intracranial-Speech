...
# Example path
# fif_path = '/Users/derekrosenzweig/Documents/GitHub/iEEG-Speech/derivatives/mega/preprocessed/70-150Hz/AttFast_run-01_concatenated_raw.fif'
# Note, the only parameters that I am inputting are 'stim' and 'run'
# As only subjects are concatenated into a single fif file
...
fig_path = os.path.join(base_path, 'vis', freq, 'TRF')
...
for sub in subjects:
    for ses, stims in session_stimuli.items():
        for stim in stims:
            for run in runs:
                # Construct file path
                fif_path = os.path.join(base_path, 'derivatives', 'mega', pro, freq, f'{stim}_{run}.fif')
                ...
                word_features = ['word_onset', 'Duration', 'NMorph', 'Syllables', 'Freq_HAL',
                ...
                parent_fig_path = os.path.join(base_path, 'vis', 'mega', freq, 'TRF')
                ...
                elec_tsv_path = os.path.join(parent_fig_path, 'tsv', f'{sub}_{stim}_{run}_{freq}_top_electrodes.tsv')
                ...
                plt.savefig(os.path.join(parent_fig_path, 'hist', f'{sub}_{stim}_{run}_{freq}_histogram.png'))
                plt.show()
                ...
                for ei, elec in enumerate(best_elecs[::3][:len(axs)]):
                    ...
                    axs[ei].axis('off')
                plt.legend()
                plt.savefig(os.path.join(parent_fig_path, 'predict', f'{sub}_{stim}_{run}_{freq}_predictions.png'))
                plt.show()
                ...
                plt.savefig(os.path.join(parent_fig_path, 'coefs', f'{sub}_{stim}_{run}_{freq}_coefs.png'))
                plt.show()
